      ...
      <section className="section-padding pt-32">
        <div className="container-custom">
          <div className="max-w-3xl">
            <span className="text-primary text-sm font-medium mb-4 block">Портфолио</span>
        ...
        <div className="container-custom">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <div key={project.id} className="portfolio-project-card floating-card overflow-hidden group flex flex-col">
                <div className="aspect-video relative overflow-hidden bg-muted/20 flex-shrink-0">
                  <Image
                    src={project.image}
                    alt={project.title}
                    fill
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <span className="text-xs text-primary font-medium transition-all duration-300 group-hover:brightness-125 group-hover:drop-shadow-[0_0_8px_hsl(var(--primary)/0.6)]">
                ...
                <div className="p-6 flex flex-col flex-1 min-h-0">
                  <div className="mb-4">
                    <span className="text-xs text-muted-foreground block mb-1">Задача</span>
                    <p className="text-sm">{project.task}</p>
                  </div>
                  <div className="mb-4">
                    <span className="text-xs text-muted-foreground block mb-1">Решение</span>
                    <p className="text-sm text-muted-foreground">{project.solution}</p>
                  </div>
                  <div className="mt-auto pt-2">
                    <span className="text-xs text-muted-foreground block mb-2">Результаты</span>
                    <div className="flex flex-wrap gap-2">
                      {project.results.map((result, idx) => (
                        <span
                          key={idx}
                          className="text-xs px-3 py-1 rounded-full bg-primary/10 text-primary transition-all duration-300 group-hover:brightness-125 group-hover:drop-shadow-[0_0_6px_hsl(var(--primary)/0.5)]"
                        >
                          {result}
                        </span>
                      ))}
